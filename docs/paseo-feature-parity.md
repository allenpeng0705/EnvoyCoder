...
| 2 | **Resume a run** | resume from the UI, per agent | wire + daemon ✅; UI ✅ — idle send says Resume when the transcript has a resumable `run.session` and the agent advertises resume (`composer/resume.ts`, TaskPane → `startRun({ resume: true })`) | ✅ done |
| 3 | **The phone pairs and connects** | QR scan → host list → run list → answer approvals | hello shape fixed ✅; session store ✅; Flutter M4 ✅ (secure storage, QR/paste/TCP/SSH dial, run list, approvals) | live phone recording of an approval answer (manual) |
...
| 9 | **Diff / review surface** | changed files, commits, PR create/merge, forges, inline review | `run.diff` emitted from write/edit paths; still owed: a diff viewer, git in the daemon |
