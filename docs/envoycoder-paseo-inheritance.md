# What EnvoyCoder takes from Paseo
...
|---|---|---|
| A local **daemon** owns agents and state; clients connect to it (`packages/server`, `packages/protocol`) | `apps/desktop/src/daemon/` + `@envoycoder/protocol` | the only shape in which closing a window does not kill a running agent |
| **Normalised streaming**, not passthrough: an event union for turns, tool calls, usage, permissions (`agent-sdk-types.ts:419-475`) | `RUN_EVENT_KINDS` in `@envoycoder/protocol` | a client renders a transcript and a diff; only the daemon knows which harness produced them |
...
| **Provider abstraction as a contract**, not a switch: `AgentClient` + `AgentSession` with capabilities (`agent-sdk-types.ts:660-803`) | `@envoycoder/agent-catalog` | the catalogue is data; the adapter is code. Capabilities decide what the UI may promise |
...
* **Settings in two scopes**: app-global and per-host. Ours adds a third level, per project, because
  the project is where defaults belong (`docs/envoycoder-ui.md`).
...
`docs/envoycoder-networking.md` §4.
...
products coexist without reading each other's data (`docs/envoycoder-design.md` §6).

## 5. What we looked at and did not take

* **Electron.** Paseo's desktop is Electron 44, reusing its own binary as Node. EnvoyCoder uses
...
* **An Expo app for mobile.** EnvoyCoder's phone app is Flutter, matching EnvoyGo, which is already
