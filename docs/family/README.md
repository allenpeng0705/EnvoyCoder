...
EnvoyCoder is built to a standard that lives in **another repository**. These three files are copies
...
| [`envoymesh-multi-product-design.md`](envoymesh-multi-product-design.md) | What happens when two products share one machine | The rules EnvoyCoder must not break: one node owner at a time and everyone else attaches (D2), share the local engine and never the cloud (D3), and the harness is a peer each product clones rather than something EnvoyMesh distributes (D4). Also the measurements — a node costs ~650 MB — behind this product running its own host |
| [`envoymesh-refactoring-plan.md`](envoymesh-refactoring-plan.md) | EnvoyMesh's own module refactor | Background, not product design. It is here because the reusable / product-bound split **is** the interface EnvoyCoder consumes, and it explains why `@envoymesh/api/core` exists and why the bare `@envoymesh/api` barrel is off limits. EnvoyCoder appears in it only as motivation (§1, §11) |
...
- **The source moved on.** The body still matches what was copied, but EnvoyMesh's file has changed
  ...
  document being edited in EnvoyMesh is no reason for EnvoyCoder's build to stop. The check reports
