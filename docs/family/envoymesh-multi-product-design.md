...
### The model lease — decided: one agreed shared model (2026-09-13)

§8 left this open because it is a policy, and the owner picked the first of the three options: **every product on a machine uses the same local model**, and a request for a different one is **refused with something a user can act on** rather than queued or silently degraded. One `llama-server` serves one model; nothing here pretends otherwise.
...
The path this replaced is worth naming, because it was wrong in a way that would have been hard to notice: the loser adopted whatever engine answered the port **without comparing models**, so a second product set to a different model would have had its inference answered by the first product's model. Under a lease that is the one outcome worse than refusing.

**Two caveats from the review round, closed.**

1. **The embeddings contention branch is now driven end-to-end.** `apps/node/test/envoy-local-embed-runtime.test.ts` gained the scaffolding the reviewer said was missing (a fake `llama-server`, a sparse model, a stubbed engine that models "the port is free, then our engine answers"): four tests cover holding the claim through a successful start, refusing to spawn while another live process holds it, refusing a different model, and taking over a stale claim. Writing them found a defect of the same class the reviewer had flagged in the watchdog: the **pre-acquire `stopEmbedListenerHard` also kills every non-self listener on the port**, so a start that was only going to wait for (or refuse) a live holder's engine would first destroy it. Cleanup is now gated on the claim, and the test asserts *no signal was sent to any pid* (with signal 0 kept real, because that is the liveness probe — mocking it unconditionally made every dead pid look alive and broke the takeover test, which is how the mistake was caught).
2. **pid reuse and the asset-root race.** A claim now records the `host` and the holder's own start time (`Date.now() - process.uptime()`). A claim from another host is treated as stale (a home on a network mount must not block this machine), and when the OS can report a pid's start time (Linux `/proc/<pid>/stat` + `btime`) a disagreement beyond five seconds means the pid was reused, so the next start takes the claim over instead of waiting out the whole startup timeout for an unrelated program. macOS and Windows have no cheap equivalent, so there the check degrades to plain pid liveness — stated rather than implied. For the first-install race (two processes resolving different asset roots, therefore different locks), the mitigation is the port arbiter above: whichever process gets there first owns the port, and the other adopts that engine instead of fighting for the bind. **The sticky resolution is now done too** (below), so the three caveats this entry left open are closed rather than carried.

### The three remaining caveats, closed (2026-09-13)

Each of these was on record as open rather than dropped, so closing them means the record changes rather than grows.

**1. The engine asset root is now decided once per home.** `localEngineAssetsDir()` answered from `exists()` per process per call, and that is the race: a process resolving while only the legacy copy existed adopted it, and a process starting after something created the shared root picked *that* — two different lock files for the same engine port, which is precisely the case the spawn lock exists to prevent, because neither process can see the other's claim. `resolveEngineRoot()` now records the answer in `<home>/runtime/engine-root.json` (`dir`, `reason`, `decidedAt`) and every later resolution obeys it, whatever has appeared on disk since. The rule itself still lives in `localEngineAssetsDir` — the sticky layer calls it rather than restating it, so the two cannot drift.

Two design points that took a second attempt: a recorded answer is **obeyed even when its directory does not exist yet** (the normal state of a fresh install — the shared root is created by the first download, *after* the decision), because invalidating the decision on a missing directory reopens the race during exactly that window; and the only thing that changes a decision is deleting the marker, which is an explicit act rather than something a version upgrade does behind the user's back. `apps/node/src/engine-root.ts` owns the marker and the sentinel guard — a host with no profile must not record a decision in `/tmp/unknown/runtime/` — and both runtimes *and the process-exit handler* resolve through it, which is what makes the release path and the lock path agree by construction rather than by two copies of a rule.

**2. The pid-reuse guard works on the platform this is developed on.** It was Linux-only (`/proc/<pid>/stat` + `btime`), which meant the check was exercised on CI and dead on macOS. `processStartedAt()` now has a second answer: `ps -o etime=` (elapsed), subtracted from now. `etime` is `[[dd-]hh:]mm:ss` — **locale-independent**, unlike `ps -o lstart=`, whose date format follows the user's locale and would silently fail to parse on a non-English machine. Windows still returns `null` and falls back to plain pid liveness, which is now a documented three-line table rather than a hedge. Aliasing with the pid-reuse comparison is ±5 s, which is why the format's one-second resolution is enough.

**3. The residual mismatch is closed, and the contradictory claim is gone.** With the sticky root, two processes on one home cannot compute different asset roots, so the "different lock files" exposure is gone; what could remain is two processes *configured* with different models, and that is the lease's business: the held-engine path refuses the mismatch by name, and the port-arbiter path refuses an unclaimed engine that serves a different model. The doc also still claimed the embeddings contention branch was untested (it had been closed in the lease entry), so that sentence now says what is true.

**Verified on the real artifact, not only in tests** — the sequence the marker exists for, run against a real node twice:

```
boot 1  (only {profile}/envoy-local exists)
  [engine] shared engine assets: <home>/profile/envoy-local (adopted-legacy, decided)
  → marker written: { dir: <home>/profile/envoy-local, reason: adopted-legacy }

then the shared root is created — a second product, or a different profile on the same home

boot 2  (both roots now exist)
  [engine] shared engine assets: <home>/profile/envoy-local (adopted-legacy, using the recorded decision)
  → marker unchanged
```

Boot 2 is the one that matters: the old rule checks `exists(shared)` *first*, so it would have switched to the shared root and the two processes would have held different claims for one engine port.
