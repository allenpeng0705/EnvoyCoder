...
| **Background service** (ours — no single Paseo row names it; it is the OS-supervised form of what `daemon.keepRunningAfterQuit` above was asking for) | the **Background service** row of EnvoyDev's own Settings (`apps/desktop/src/components/settings/SectionsService.tsx`) · one press per state the supervisor reported — *Turn on* · *Restart* · *Turn off* · *Try again* · *Refresh* | **nothing is stored here**: the value lives in the operating system's service manager — a launchd LaunchAgent, a systemd **user** unit, a per-user Task Scheduler job | runs the daemon under that supervisor so a paired phone reaches this machine while the EnvoyDev window is closed; the row shows the supervisor's measured state — off, running, installed-but-stopped, failed, unsupported, could-not-tell — and its own words last as the diagnostic line | **built**: the control exists in EnvoyDev's own Settings and the value lives in the OS supervisor, **not** in a settings file — there is no `CoderSettings` field for it. The daemon asks the supervisor while the window may only request (the three mutating calls are owner-window-only), and the service starts **at login**, never at boot | **honour-able now** |
