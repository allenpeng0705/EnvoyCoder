...
| **Background service** (ours — no single Paseo row names it; it is the OS-supervised form of what `daemon.keepRunningAfterQuit` above was asking for) | the **Background service** row of EnvoyDev's own Settings (`apps/desktop/src/components/settings/SectionsService.tsx`) · one press per state the supervisor reported — *Turn on* · *Restart* · *Stop* · *Turn off* · *Try again* · *Refresh* — plus a **Show the log** disclosure onto that daemon's log tail (`ServiceLog.tsx`) | **nothing is stored here**: the value lives in the operating system's service manager — a launchd LaunchAgent, a systemd **user** unit, a per-user Task Scheduler job | runs the daemon under that supervisor so a paired phone reaches this machine while the EnvoyDev window is closed; the row shows the supervisor's measured state — off, running, installed-but-stopped, failed, unsupported, could-not-tell — then the daemon's own restart count and last stop as evidence, with the supervisor's own words last as the diagnostic line. The disclosure reads the bounded end of the live log on demand (`coder.getDaemonLog`), names the file, and **says out loud when the tail is not the whole log** | **built**: the control exists in EnvoyDev's own Settings and the value lives in the OS supervisor, **not** in a settings file — there is no `CoderSettings` field for it. The daemon asks the supervisor while the window may only request: the four mutating calls and `coder.shutdown` are owner-window-only, and so is the log read. A paired phone may read the **state** but never the supervisor's `detail`, which names paths, argv and the home (`docs/daemon-lifecycle.md` §11). The service starts **at login**, never at boot. *Stop* ends the daemon until the next login (`coder.shutdown`); *Turn off* removes the service, and the row says which is which | **honour-able now** |
...
decisions in `agent-catalog.ts`), two groups on the page:

| group | rows from | state from |
|---|---|---|
| **Agents** | `coder.listHarnesses` (the nine we ship) then `coder.listProviders` (Added catalogue recipes / manual programs) — **one list**, same level | the daemon's probe; providers also carry each named variable with `set: true/false` |
| **Catalogue** | `coder.listCatalog` — the 38 recipes | cheap availability on list; Add materialises a provider into the Agents list |

and one form under the catalogue for a program nobody catalogued, which is what `coder.addProvider` was written for.
...
**Adding one.** Pressing *Add* on a catalogue row moves it into the Agents list as a provider with its own
...
2. **Group headers carry counts.** `Agents · 9` (built-in + Added), `Catalogue · 38`. The
