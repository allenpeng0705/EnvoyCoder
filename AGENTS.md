# EnvoyDev — working notes for agents and contributors
...
1. **The family's rules are not suggestions.** The shared home is resolved by EnvoyMesh's
   `node-core`; product state lives in `<home>/EnvoyDev/`; the harness is a *peer* we clone, never
...
2. **No `process.platform` checks in feature code.** Platform differences go in
   `@envoydev/platform`, parameterised so their branches are tested on whichever OS runs the suite.
...
6. **Tests and the bundle are different proofs.** Run `npx vitest run` *and*
   `npm run build -w @envoydev/desktop`; a green suite once shipped a broken import path.
...
* New to the family? Read `docs/envoydev-design.md` §4 (the decisions) before changing anything
  structural.
