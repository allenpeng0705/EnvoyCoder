...
export const zh: Catalogue = {
  ...
  "settings.sections.note": "这个窗口的设置，按分区排列。每个分区里都有可以查看或更改的内容。",
  ...
  "settings.agents.add.heading": "添加智能体",
  "settings.agents.add.note": "这些是 EnvoyCoder 知道如何驱动的配方。这台电脑能否运行某一个，是需要实测才能确定的事实，所以在你要求之前，这里什么都不会检查——一次只查一行，因为检查会遍历本机的程序目录，而一个打开时就全部检查的页面，会为你根本没看过的智能体消耗你的机器。",
  ...
  "settings.agents.row.add.title": "把 {agent} 加入你的智能体。它的命令行以及所需环境变量的名称都取自这一条目，与上面写的一模一样。",
  "settings.agents.row.builtIn": "EnvoyCoder 已经自带这个智能体，所以在上面的“在这台电脑上”中配置它，而不是再添加一次。",
  "settings.agents.row.needsNoInstall": "无需安装：{package} 会在首次运行时从 npm 获取。",
  ...
  "settings.agents.row.recipeEnv": "这个配方还会为该智能体设置 {names}。你自己添加的智能体只保存变量名，所以请在 EnvoyCoder 运行所在的环境中设置它们——在设置之前，这个智能体会在启动时被拒绝，而不是启动后无法使用 ACP。",
  "settings.agents.row.checked": "检查于 {when}——搜索耗时 {ms} 毫秒。",
  "settings.agents.row.checked.cached": "检查于 {when}，此后未再实测：说明智能体就在本机的结果会被记住十分钟。",
  "settings.agents.manual.heading": "不在列表中的智能体",
  "settings.agents.manual.note": "如果你已经有一个会说 Agent Client Protocol 的编码智能体，而它不在上面的目录里，就在这里声明它。EnvoyCoder 会像对待其他智能体一样启动并探测它。",
  ...
  "settings.agents.manual.env.detail": "只填名称，用逗号或换行分隔——绝不要填值。EnvoyCoder 会从守护进程运行所在的环境读取每个值，因此凭据永远不会被写入磁盘。",
  "settings.agents.manual.transport": "EnvoyCoder 应如何与它对话",
  "settings.agents.manual.transport.detail": "这里刻意没有默认值：猜错会把 Agent Client Protocol 握手发给一个永远不会回应的程序，或者把一个可用的智能体报告为不受支持。只有你能看出它是哪一种。",
  ...
  "settings.shortcuts.note": "这个窗口正在监听的每一个按键，读取自键盘层所用的同一张表。不在此列表中的按键在当前版本中不起作用。",
  ...
  "settings.machine.note": "本页的内容全部来自这个窗口连接时服务自己给出的回答。",
  ...
  "settings.machine.stateDir.detail": "这台计算机上的一个文件夹：你的设置、项目列表，以及任务保留下来的记录。",
  "settings.machine.home.title": "它视为主目录的文件夹",
  "settings.machine.home.detail": "你输入为 ~ 的路径就指这个文件夹。除此之外不会从它读取任何东西，这个窗口也不会写入它。",
  ...
  "settings.about.note": "EnvoyCoder 由两半组成——这个窗口，以及保存你的设置并运行智能体的服务——它们一起构建，所以版本应当一致。",
