...
export const zh: Catalogue = {
  ...
  "settings.agents.mine.env.title": "EnvoyCoder 的守护进程是否设置了 {name}",
  "settings.agents.mine.env.set": "已设置 {name}",
  "settings.agents.mine.env.unset": "未设置 {name}",
  ...
  "settings.agent.unchecked": "尚未检查",
  "settings.agent.checking": "正在检查…",
  "settings.agent.refused": "检查失败",
  ...
  "settings.agents.row.check": "检查这台电脑",
  "settings.agents.row.checkAgain": "重新检查",
  "settings.agents.row.check.title": "在本机查找这个智能体的程序。不会启动任何东西，也不会下载任何东西——结果会被记住十分钟。",
  ...
  "settings.agents.row.needsNoInstall": "无需安装：{package} 会在首次运行时从 npm 获取。这次检查只找到了 `npx`，并没有找到 {agent}——它还没有被下载。",
  "settings.agents.row.needsInstall": "安装 {agent} —— 之后它就会按上面的命令运行。EnvoyCoder 只负责启动程序，不会替你安装。",
  "settings.agents.row.install": "先安装 {agent}",
  ...
  "settings.agents.row.recipeEnv": "这个配方会为该智能体设置 {names}。EnvoyCoder 直接从这一条目提供它们，所以你不需要自己设置——想改掉配方里的值，导出一个即可。",
  "settings.agents.row.readyNpx": "尚未下载",
  "settings.agents.row.checked": "检查于 {when}——搜索耗时 {ms} 毫秒。",
  "settings.agents.row.checked.cached": "检查于 {when}，此后未再实测：说明智能体就在本机的结果会被记住十分钟。",
  ...
  "settings.agent.unknown": "无法检查",
  "settings.agent.ready": "就绪",
  "settings.agent.unsupported": "尚无法驱动",
  "settings.agent.needsBridge": "需要它的适配器",
  "settings.agent.provisional": "临时副本",
  ...
  "settings.agent.olderDaemon": "你的守护进程版本落后，无法判断缺的是哪一部分。重启 EnvoyCoder 以使用当前版本。",
