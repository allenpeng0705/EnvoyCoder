...
export const ja: Catalogue = {
  ...
  "settings.sections.note": "このウィンドウの設定をセクションごとに並べたものです。どのセクションにも、読むものか変更できるものがあります。",
  ...
  "settings.agents.add.heading": "エージェントを追加",
  "settings.agents.add.note": "ここにあるのは EnvoyCoder が駆動できるレシピです。このマシンで動くかどうかは実測してはじめて分かる事実なので、あなたが求めるまでここでは何も確認しません——一度に一行だけです。確認はこのマシンのプログラムフォルダを走査するので、開いたときに全部を確認する画面は、あなたが見てもいないエージェントのためにマシンを費やすことになります。",
  ...
  "settings.agents.manual.note": "Agent Client Protocol を話すコーディングエージェントをすでに持っていて、上のカタログにない場合は、ここで宣言してください。EnvoyCoder はほかのエージェントとまったく同じように起動し、調査します。",
  ...
  "settings.agents.manual.env.detail": "名前だけを、カンマまたは改行で区切ってください——値は決して書かないでください。EnvoyCoder はデーモンが動作している環境から各値を読むため、資格情報がディスクに書き込まれることはありません。",
  "settings.agents.manual.transport": "EnvoyCoder がどのように話すか",
  "settings.agents.manual.transport.detail": "ここに既定値がないのは意図的です。推測すると、決して応答しないプログラムに Agent Client Protocol のハンドシェイクを送ったり、動作するエージェントを非対応と報告したりします。どちらなのか分かるのはあなただけです。",
  ...
  "settings.shortcuts.note": "このウィンドウが待ち受けているすべてのキー。キーボード層が読むのと同じ表から読み出しています。ここにないキーは、このバージョンでは何も行いません。",
  ...
  "settings.machine.note": "このページの内容はすべて、このウィンドウが接続したときにサービス自身が答えたものです。",
  ...
  "settings.machine.stateDir.detail": "このコンピューター上の一つのフォルダーです。設定、プロジェクト一覧、そしてタスクが残す記録が入ります。",
  "settings.machine.home.title": "ホームとして扱うフォルダー",
  "settings.machine.home.detail": "~ と入力したパスはこのフォルダーを指します。それ以外にここから読み取るものはなく、このウィンドウがここに書き込むこともありません。",
  ...
  "settings.about.note": "EnvoyCoder は二つの半分——このウィンドウと、設定を保存しエージェントを実行するサービス——でできており、一緒にビルドされるのでバージョンは一致するはずです。",
