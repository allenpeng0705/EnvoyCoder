...
export const ja: Catalogue = {
  ...
  "error.connectorNotFetchable": "{harness} には npm で公開されたコネクタがないため、EnvoyCoder は取得できません。代わりにインストールしてください。",
  ...
  "settings.agent.verdict.legacy.line": "EnvoyCoder が 1 世代前です",
  "settings.agent.verdict.legacy.why":
    "このウィンドウは古い EnvoyCoder デーモンと通信しているため、このマシンで {agent} をどう使えるかを判断できません。あなたのマシンに欠けているものはありません — アプリの二つの半分が別のビルドなだけです。",
  "settings.agent.verdict.unlooked.line": "EnvoyCoder はこのマシンを確認できませんでした",
  "settings.agent.verdict.unlooked.why":
    "これは私たち側の不足であり、{agent} についての判定ではありません。EnvoyCoder はこのマシンのプログラムフォルダーを読めなかったため、エージェントがインストールされているかどうかを何も知りません。この行を見てインストールすべきものはありません。",
  "settings.agent.verdict.app.restart": "EnvoyCoder を再起動",
  "settings.agent.verdict.connector.lead": "インストール済み — コネクターが必要です",
  "settings.agent.verdict.connector.why": "{agent} はインストールされています。EnvoyCoder がこれを駆動するにはコネクターが必要で、欠けているのはそれだけです。",
  ...
  "settings.agent.verdict.gap.line": "EnvoyCoder はまだこのエージェントを駆動できません",
  "settings.agent.verdict.gap.why":
    "これは EnvoyCoder 側の不足で、プログラムが足りないわけではありません。{agent} はこのビルドにアダプターがない種類のインターフェースで接続されており、もう一度インストールしても何も変わりません。あなたのマシンに問題はありません。",
  ...
  "settings.agent.verdict.env.why":
    "{agent} はインストールされています。EnvoyCoder のデーモンを起動した環境で {names} を設定してください — 値はあなたからしか来ません。EnvoyCoder は認証情報を決して書き留めないからです。",
  ...
  "settings.agent.fact.verified.never": "まだです — EnvoyCoder はこれを知るためにエージェントを起動するので、このページを開いたときではなく、タスクが実行されたときに分かります。",
  "settings.agent.fact.publishes": "公開しているもの",
  "settings.agent.fact.env.recipe": "このレシピが用意します",
  "settings.agent.fact.env.set": "EnvoyCoder の環境で設定済み",
