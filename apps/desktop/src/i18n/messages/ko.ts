...
export const ko: Catalogue = {
  ...
  "settings.sections.note": "이 창의 설정을 섹션별로 모았습니다. 모든 섹션에 읽거나 바꿀 것이 있습니다.",
  ...
  "settings.agents.add.heading": "에이전트 추가",
  "settings.agents.add.note": "여기 있는 것은 EnvoyCoder가 구동할 수 있는 레시피입니다. 이 컴퓨터에서 실행되는지는 측정해야 아는 사실이므로, 요청하기 전에는 아무것도 확인하지 않습니다 — 한 번에 한 줄씩입니다. 확인은 이 컴퓨터의 프로그램 폴더를 훑기 때문에, 열 때 전부 확인하는 화면은 보지도 않은 에이전트를 위해 컴퓨터를 쓰는 셈입니다.",
  ...
  "settings.agents.manual.note": "Agent Client Protocol을 사용하는 코딩 에이전트가 이미 있고 위 카탈로그에 없다면 여기에서 선언하세요. EnvoyCoder는 다른 에이전트와 똑같이 실행하고 살펴봅니다.",
  ...
  "settings.agents.manual.env.detail": "이름만, 쉼표나 줄바꿈으로 구분해 적으세요 — 값은 절대 적지 마세요. EnvoyCoder는 데몬이 실행되는 환경에서 각 값을 읽으므로 자격 증명이 디스크에 기록되는 일은 없습니다.",
  "settings.agents.manual.transport": "EnvoyCoder가 어떻게 말해야 하는지",
  "settings.agents.manual.transport.detail": "여기에 기본값이 없는 것은 의도적입니다. 추측하면 응답하지 않을 프로그램에 Agent Client Protocol 핸드셰이크를 보내거나, 작동하는 에이전트를 지원되지 않는다고 보고하게 됩니다. 어느 쪽인지 알 수 있는 것은 당신뿐입니다.",
  ...
  "settings.shortcuts.note": "이 창이 듣고 있는 모든 키입니다. 키보드 계층이 읽는 것과 같은 표에서 읽어 옵니다. 여기에 없는 키는 이 버전에서 아무 일도 하지 않습니다.",
  ...
  "settings.machine.note": "이 페이지의 모든 내용은 이 창이 연결될 때 서비스가 스스로 답한 것입니다.",
  ...
  "settings.machine.stateDir.detail": "이 컴퓨터의 한 폴더입니다. 설정, 프로젝트 목록, 작업이 남기는 기록이 들어 있습니다.",
  "settings.machine.home.title": "홈으로 취급하는 폴더",
  "settings.machine.home.detail": "~ 로 입력한 경로는 이 폴더를 뜻합니다. 그 밖에는 여기서 읽는 것이 없고, 이 창이 여기에 쓰는 것도 없습니다.",
  ...
  "settings.about.note": "EnvoyCoder는 두 부분——이 창과, 설정을 저장하고 에이전트를 실행하는 서비스——으로 이루어져 있고 함께 빌드되므로 버전이 같아야 합니다.",
